...
const useLiveData = () => {
    ...
    const { data: crewMembers, isLoading: loadingCrew, isError: errorCrew } = api.crewMembers.getAll.useQuery(); //could be inefficient to fetch all crew members and projects for every blueprint - TODO fix
    const { data: projects, isLoading: loadingProjects, isError: errorProjects } = api.projects.getAll.useQuery();
