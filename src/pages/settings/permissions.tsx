  ...
  return (
    <SettingsLayout>
      <div className="min-h-[30vh]">
        <div className="flex items-center justify-between gap-2 border-b border-zinc-700 pb-2 font-semibold text-zinc-200">
          <input
            type="text"
            autoFocus
            ...
            className="w-full rounded border border-zinc-500 bg-zinc-700 p-2 text-zinc-200 outline-none transition-all duration-100 hover:ring-1 hover:ring-amber-600 focus:ring-2 focus:ring-amber-600"
          />
          <PermissionsModal
            newPermission={true}
...
const PermissionsListItem: React.FC<{
          ...
          <div className="flex flex-col">
            <div className="flex flex-wrap items-center justify-start gap-1">
              ...
              {keywords?.map((keyword) => (
                <div
                  key={keyword.id}
                  className={`flex items-center whitespace-nowrap rounded-xl border px-2 text-xs`}
                >
                  {keyword.name}
                </div>
              ))}
...
const PermissionsModal: React.FC<{
  ...
  keywordData?: PermissionKey[];
}> = ({ newPermission, children, className, permissionData, keywordData }) => {
...
const KeywordsMultiSelect: React.FC<{
  ...
  const { data, isLoading, isError } =
    api.permissionsKeys.getAllKeys.useQuery();
